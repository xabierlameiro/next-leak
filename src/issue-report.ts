import path from "node:path";
import type { FindingAttribution } from "./attribution.js";
import { assessPeakPressure, describePeakPressure } from "./peak-pressure.js";
import type { MeasuredRoute, RunReport } from "./runner.js";

const MB = 1024 * 1024;

/** How a finding's owner reads in the draft, or "unattributed". */
function ownerLabel(attribution: FindingAttribution | undefined): string {
  if (attribution === undefined || attribution.owner === "unattributed") {
    return "unattributed";
  }
  const source = attribution.source ? ` — \`${attribution.source}\`` : "";
  const packageName = attribution.packageName ? ` — ${attribution.packageName}` : "";
  return `${attribution.owner}${source}${packageName}`;
}

/** Who the warning banner points at when the leak is not upstream's. */
function blamedParty(owner: string, culprit: { source?: string | null; packageName?: string | null } | undefined): string {
  if (owner === "app") {
    return `**your own code** (\`${culprit?.source ?? "app code"}\`)`;
  }
  return `the dependency **${culprit?.packageName ?? "a dependency"}**`;
}

function evidenceRows(route: MeasuredRoute): string {
  const findings = [...(route.diff?.grownNodes ?? []), ...(route.diff?.newNodes ?? [])];
  return findings
    .slice(0, 6)
    .map((finding, index) => {
      const attribution = route.attribution?.findings[index];
      const owner = ownerLabel(attribution);
      return (
        `- **${finding.kind}** \`[${finding.nodeType}] ${finding.name}\` ` +
        `${(finding.retainedBytes / MB).toFixed(2)} MB retained (${owner})\n` +
        `  - retainers: \`${finding.retainerChain || "(none)"}\``
      );
    })
    .join("\n");
}

/**
 * Issue draft in the Next.js bug-template section order. The attribution
 * gates the preamble: app/dependency-owned leaks warn against filing
 * upstream; framework/unattributed leaks read as an upstream-ready draft.
 */
export function renderIssueMarkdown(route: MeasuredRoute, run: RunReport): string {
  const environment = run.environment;
  const parameters = run.parameters;
  const owner = route.attribution?.route.owner ?? "unattributed";
  const culprit = route.attribution?.route;

  const preamble =
    owner === "app" || owner === "dependency"
      ? `> [!WARNING]\n` +
        `> next-leak attributes this leak to ${blamedParty(owner, culprit)}. ` +
        `Fix or report it there — do **not** file this against Next.js.\n\n`
      : "";

  // Stated up front rather than left for a maintainer to discover. A draft
  // that hides what its own run could not establish is worth less than one
  // that says so: the first reviewer to spot it discounts everything else.
  // A cache being filled by the measurement is an alternative explanation for
  // the whole curve, so it goes above the caveats rather than inside them: a
  // reader who finds it further down has already read the growth figure as a
  // leak. Measured on a `use cache` route on 2026-08-27, this accounted for
  // +603 MB/1000 requests against +88 MB once the payload was removed.
  const cacheDisclosure =
    route.trend.cacheDriven === true
      ? `> [!IMPORTANT]\n` +
        `> The load drove this route with cache keys it had never served, so ` +
        `part of the growth below is the cache filling up rather than a leak. ` +
        `Re-measure with a bounded key set (\`{n%N}\` in \`next-leak.config.json\`) ` +
        `before filing this.\n\n`
      : "";

  const caveats =
    route.confidence.warnings.length === 0
      ? ""
      : `\n### Measurement caveats\n\n` +
        `next-leak audits its own run and reports these limits. They do not ` +
        `overturn the verdict above, but they bound how much weight it carries:\n\n` +
        route.confidence.warnings.map((warning) => `- ${warning.detail}`).join("\n") +
        `\n`;

  // Retention is the claim; the peak is what a container is sized against.
  // A maintainer reading a leak report is better served by both.
  const retainedHeap = route.memorySamples.at(-1)?.heapUsed;
  const pressure =
    retainedHeap === undefined || route.peaks === undefined
      ? null
      : assessPeakPressure({
          peaks: route.peaks,
          retainedHeapBytes: retainedHeap,
          maxOldSpaceMb: parameters.maxOldSpaceMb,
        });
  const peakSection =
    pressure === null
      ? ""
      : `\n### Peak memory under load\n\n` +
        `Sampled during the load phases, without forcing collection: ` +
        `${describePeakPressure(pressure)}. Per cycle:\n\n` +
        route.peaks
          .filter((peak) => peak.polls > 0)
          .map(
            (peak) =>
              `- ${peak.phase}: heap ${(peak.heapUsed / MB).toFixed(1)} MB, ` +
              `external ${(peak.external / MB).toFixed(1)} MB, ` +
              `arrayBuffers ${(peak.arrayBuffers / MB).toFixed(1)} MB, ` +
              `rss ${(peak.rss / MB).toFixed(1)} MB`
          )
          .join("\n") +
        `\n`;

  const curve = route.samples.map((sample) => (sample / MB).toFixed(1)).join(" → ");
  const deltas = route.trend.deltas.map((delta) => `+${(delta / MB).toFixed(2)}`).join(", ");
  const signatures = route.signatures
    .map(
      (signature) =>
        `- ${signature.historical ? "(historical) " : ""}${signature.title} — ${signature.issue}`
    )
    .join("\n");

  return `${preamble}${cacheDisclosure}# Memory leak on route \`${route.route}\`

Report generated by \`next-leak\` — every number below is reproducible from
the raw heap snapshots referenced at the bottom.

### Provide environment information

\`\`\`
Node.js: ${environment.nodeVersion}
OS: ${environment.platform} ${environment.arch}${environment.cpuModel ? ` (${environment.cpuModel})` : ""}
Memory: ${(environment.totalMemoryBytes / (1024 * MB)).toFixed(0)} GB
Next.js: ${environment.nextVersion ?? "unknown"}
next-leak: ${environment.nextLeakVersion}
Deployment: output "standalone", node --expose-gc --max-old-space-size=${parameters.maxOldSpaceMb}
\`\`\`

### To Reproduce

1. Build the app with \`output: "standalone"\` and run:
   \`\`\`
   npx next-leak <app-dir>
   \`\`\`
2. next-leak boots \`.next/standalone/server.js\` in a fresh process per route and runs, against \`${route.requestPath}\`:
   warm-up ${parameters.warmupRequests} requests → forced GC → baseline heap snapshot →
   ${parameters.cycles} × [${parameters.loadRequests} requests at ${parameters.connections} connections → ${(parameters.idleMs / 1000).toFixed(0)}s idle → forced GC → post-GC sample] → final snapshot.
3. A cycle counts as growing when it retains at least ${(parameters.minGrowthPerCycle / 1024).toFixed(0)} KiB
   (the gate for this run's ${parameters.loadRequests} requests per cycle); the warm-up cycle is excluded.

### Current vs. Expected behavior

**Current:** retained heap after forced GC keeps growing every cycle —
${curve} MB (per-cycle deltas ${deltas} MB), ≈ **${(route.growthPer1000Requests / MB).toFixed(2)} MB per 1000 requests**.
The first cycle is excluded from the verdict (engine warm-up).

**Expected:** retained heap flattens after warm-up (plateau), as measured on this app's healthy routes.

### Heap evidence

${evidenceRows(route) || "- (no findings above thresholds)"}
${signatures === "" ? "" : `\n**Matched known causes:**\n${signatures}\n`}${peakSection}${caveats}
### Verify it yourself

Raw snapshots (Chrome DevTools → Memory → Load, compare baseline vs after):

- \`${path.basename(route.baselineSnapshot)}\` / \`${path.basename(route.afterSnapshot)}\` in \`${run.workDir}\`
- machine-readable data: \`run.json\`, curves: \`report.html\`
`;
}
